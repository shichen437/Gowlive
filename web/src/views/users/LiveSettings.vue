        ...
        <div class="grid gap-2">
          <div class="flex items-center justify-between">
            <Label for="archive-strategy" class="flex flex-col space-y-1">
              <span class="text-md">{{
                t('user.liveSettings.archive.title')
              }}</span>
            </Label>
            <Select
              id="archive-strategy"
              v-model="archiveStrategy"
              @update:model-value="updateSetting('sk_archive_strategy', $event)"
            >
              <SelectTrigger class="w-[330px]">
                <SelectValue
                  :placeholder="t('user.liveSettings.archive.placeholder')"
                />
              </SelectTrigger>
              <SelectContent>
                <SelectGroup>
                  <SelectItem :value="0"
                    >{{ t('user.liveSettings.archive.values.monthly') }}
                  </SelectItem>
                  <SelectItem :value="1">{{
                    t('user.liveSettings.archive.values.daily')
                  }}</SelectItem>
                  <SelectItem :value="2"
                    >{{ t('user.liveSettings.archive.values.monthlyAndDaily') }}
                  </SelectItem>
                  <SelectItem :value="3">{{
                    t('user.liveSettings.archive.values.none')
                  }}</SelectItem>
                </SelectGroup>
              </SelectContent>
            </Select>
          ...
          <div class="flex items-center justify-between">
            <Label for="filename-template" class="flex flex-col space-y-1">
              <span class="text-md">{{
                t('user.liveSettings.filenameTemplate.title')
              }}</span>
            </Label>
            <Select
              ...
              @update:model-value="
                updateSetting('sk_filename_template', $event)
              "
            >
              <SelectTrigger class="w-[330px]">
                <SelectValue
                  :placeholder="
                    t('user.liveSettings.filenameTemplate.placeholder')
                  "
                />
              </SelectTrigger>
              <SelectContent>
                <SelectGroup>
                  <SelectItem :value="0"
                    >[2025-01-01 00:00:00][主播名称][房间名称]</SelectItem
                  >
                  <SelectItem :value="1"
                    >[主播名称][房间名称][2025-01-01 00:00:00]</SelectItem
                  >
                  <SelectItem :value="2"
                    >2025-01-01 00:00:00_主播名称_房间名称</SelectItem
                  >
                  <SelectItem :value="3"
                    >主播名称_房间名称_2025-01-01 00:00:00</SelectItem
                  >
                  <SelectItem :value="4"
                    >[2025-01-01 00:00:00][主播名称]</SelectItem
                  >
                  <SelectItem :value="5"
                    >[主播名称][2025-01-01 00:00:00]</SelectItem
                  >
                  <SelectItem :value="6"
                    >2025-01-01 00:00:00_主播名称</SelectItem
                  >
                  <SelectItem :value="7"
                    >主播名称_2025-01-01 00:00:00</SelectItem
                  >
                </SelectGroup>
              </SelectContent>
            </Select>
      ...
      <CardContent>
        <div class="grid gap-2">
          <div class="flex items-center justify-between">
            ...
            <Select
              id="disk-protection"
              v-model="diskProtection"
              @update:model-value="updateSetting('sk_disk_protection', $event)"
            >
              <SelectTrigger class="w-[330px]">
                <SelectValue
                  :placeholder="
                    t('user.liveSettings.diskProtected.placeholder')
                  "
                />
              </SelectTrigger>
              <SelectContent>
                <SelectGroup>
                  <SelectItem :value="0">{{
                    t('user.profile.fileds.disabled')
                  }}</SelectItem>
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <Label
                for="auto-clean-little-file"
                class="flex flex-col space-y-1"
              >
                <span class="text-md">{{
                  t('user.liveSettings.autoCleanLittleFile.title')
                }}</span>
              </Label>
            </div>
            <Select
              id="auto-clean-little-file"
              v-model="autoCleanLittleFile"
              @update:model-value="
                updateSetting('sk_auto_clean_little_file', $event)
              "
            >
              <SelectTrigger class="w-[330px]">
                <SelectValue
                  :placeholder="
                    t('user.liveSettings.autoCleanLittleFile.placeholder')
                  "
                />
              </SelectTrigger>
              <SelectContent>
      ...
      <CardContent>
        <div class="grid gap-2">
          <div class="flex items-center justify-between">
            ...
            <Switch
              id="fixed-resolution"
              :checked="fixedResolution"
              v-model="fixedResolution"
        ...
        <div class="grid gap-2">
          <div class="flex items-center justify-between">
            <Label for="live-end-notify" class="flex flex-col space-y-1">
              <span class="text-md">{{
                t('user.liveSettings.liveCloseNotice')
              }}</span>
            </Label>
            <Switch
              id="live-end-notify"
              :checked="liveEndNotify"
              v-model="liveEndNotify"
...
onMounted(async () => {
  try {
    const result: Record<string, number> = await fetchSetting(
      'sk_live_end_notify,sk_filename_template,sk_archive_strategy,sk_disk_protection,sk_auto_clean_little_file,sk_fixed_resolution',
    );
