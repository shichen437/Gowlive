        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="flex items-center justify-between">
            <Label for="data-sync-enable" class="flex flex-col space-y-1">
              <span class="text-md">{{
                t('user.syncSettings.enable.title')
              }}</span>
            </Label>
            <Switch
              id="data-sync-enable"
              :checked="dataSyncEnable"
              v-model="dataSyncEnable"
          ...
          <div class="flex items-center justify-between">
            <Label for="data-sync-failed-retry" class="flex flex-col space-y-1">
              <span class="text-md">{{
                t('user.syncSettings.failedRetry')
              }}</span>
            </Label>
            <Switch
              id="data-sync-failed-retry"
              :checked="dataSyncFailedRetry"
              v-model="dataSyncFailedRetry"
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="flex items-center justify-between">
            <Label for="data-sync-auto-delete" class="flex flex-col space-y-1">
              <span class="text-md">{{
                t('user.syncSettings.deleteOnFinish')
              }}</span>
            </Label>
            <Switch
              id="data-sync-auto-delete"
              :checked="dataSyncAutoDelete"
              v-model="dataSyncAutoDelete"
              @update:checked="
                updateSetting('sk_data_sync_auto_delete', $event)
              "
              :disabled="openlist.status === 0"
            />
          </div>
...
async function fetchOpenlistStatus() {
  ...
  try {
    ...
    if (res.code === 0 && statusInfo) {
      ...
      openlist.value = {
        status: 400,
        text: t('user.syncSettings.connectionFailed'),
        color: 'text-red-500',
      };
    }
  } catch (error) {
...
onMounted(async () => {
  try {
    const result: Record<string, number> = await fetchSetting(
      'sk_data_sync_enable,sk_data_sync_failed_retry,sk_data_sync_auto_delete',
    );
    ...
    const defaultStatusInfo = {
      text: t('user.syncSettings.notEnabled'),
      color: 'text-gray-500',
    };
