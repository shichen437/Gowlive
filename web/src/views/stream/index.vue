<template>
      ...
      <div class="flex items-center space-x-2">
        <div class="flex items-center">
          <Button
            ...
            :aria-label="
              displayMode === 'list'
                ? t('stream.rooms.buttons.cardView')
                : t('stream.rooms.buttons.listView')
            "
          >
        ...
        <SortRoomDropDownMenu
          v-model:sort="sort"
          @update:sort="handleSortChange"
        />
        <FilterRoomDropDownMenu
        ...
        <TableHeader>
          <TableRow>
            <TableHead class="text-center">{{
              t('stream.common.fields.anchorName')
            }}</TableHead>
            <TableHead class="text-center">{{
              t('stream.rooms.fields.roomName')
            }}</TableHead>
            <TableHead class="text-center">{{
              t('common.fields.platform')
            }}</TableHead>
            <TableHead class="text-center">{{
              t('stream.rooms.fields.monitorStatus.title')
            }}</TableHead>
            <TableHead class="text-center">{{
          ...
          <template v-if="rooms.length > 0">
            ...
            >
              ...
              <TableCell
                class="text-center"
                :class="
                  getStatusColor(room.status, room.isRecording, room.isLiving)
                "
                >{{
              ...
              <TableCell class="text-center space-x-2">
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  @click="openEditRoomModal(room.liveId)"
                >
                  <Pencil class="w-4 h-4" />
                </Button>

                <RoomActionDropdownMenu
                  :room="room"
                  @top-room="handleTopRoom"
                  @un-top-room="handleUnTopRoom"
                  @go-to-folder="handleGoToRoomFolder"
                  @delete-room="openConfirmModal"
                  @stream-preview="handlePreview"
                />
              </TableCell>
            </TableRow>
          </template>
    ...
    >
      <template v-if="rooms.length > 0">
        ...
        >
          ...
          >
            <CardTitle class="text-lg truncate" :title="room.anchor">{{
              room.anchor
            }}</CardTitle>
            <div class="flex items-center space-x-2">
              <Badge variant="outline">{{
  ...
  <ConfirmModal
    ...
    :onOpenChange="(open: any) => (showStartConfirmModal = open)"
    :onConfirm="handleStartRoom"
    :title="t('stream.rooms.startMonitor.title')"
    :description="t('stream.rooms.startMonitor.desc')"
  />
  <ConfirmModal
    :open="showStopConfirmModal"
    :onOpenChange="(open: any) => (showStopConfirmModal = open)"
    :onConfirm="handleStopRoom"
    :title="t('stream.rooms.stopMonitor.title')"
