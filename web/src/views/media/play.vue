<template>
  <div class="flex h-screen">
    <div class="w-3/4 flex flex-col p-4">
      ...
      <div
        class="grow flex items-center justify-center rounded-lg bg-black/5 overflow-hidden"
      >
        <VideoPlayer
          ref="playerRef"
          v-if="videoUrl"
        ...
        <div v-else class="text-muted-foreground">
          {{ t('media.play.toast.loadVideoErr') }}
        </div>
      </div>
    </div>
            ...
            >
              <TableRow
                ...
                :class="{
                  ...
                  'cursor-pointer': canPlay(file) && file.filename !== filename,
                }"
              >
                <TableCell
                  class="w-60 max-w-60 whitespace-nowrap overflow-hidden text-ellipsis"
                >
                  {{ file.filename }}
                </TableCell>
              </TableRow>
  ...
  <Dialog :open="isClipModalOpen" @update:open="isClipModalOpen = $event">
    <DialogContent class="sm:max-w-[500px]">
          ...
          <div class="space-y-2">
            ...
            <div class="flex gap-2">
              ...
              <Button
                ...
                :title="t('media.play.clip.fields.setStartTime')"
              >
          ...
          <div class="space-y-2">
            ...
            <div class="flex gap-2">
              ...
              <Button
                ...
                :title="t('media.play.clip.fields.setEndTime')"
              >
                <MapPin class="w-4 h-4" />
              </Button>
            </div>
      ...
      <DialogFooter>
        <Button variant="outline" @click="isClipModalOpen = false">{{
          t('common.operation.cancel')
        }}</Button>
        <Button @click="handleClipSubmit" :disabled="isClipping">
          {{
            isClipping
              ? t('media.play.clip.clipping')
              : t('media.play.clip.startClip')
          }}
        </Button>
      </DialogFooter>
    </DialogContent>
...
const handleItemClick = (file: FileInfo) => {
  if (canPlay(file)) {
    ...
    router.replace({
      query: { path: currentPath.value, filename: file.filename },
    });
  }
};
