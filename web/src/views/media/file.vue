<template>
      ...
      <Table>
        ...
        <TableBody>
          <TableRow v-for="file in files" :key="file.filename">
            ...
            <TableCell
              class="text-center truncate max-w-xs mx-auto"
              @click="handleItemClick(file)"
              :class="{ 'cursor-pointer hover:bg-muted/50': file.isFolder }"
              >{{ file.filename }}
            </TableCell>
            <TableCell
              class="text-center md:w-[100px]"
              @click="handleItemClick(file)"
              :class="{ 'cursor-pointer hover:bg-muted/50': file.isFolder }"
              >{{ file.isFolder ? '-' : formatBytes(file.size) }}</TableCell
            >
            <TableCell
              class="text-center md:w-[220px]"
              @click="handleItemClick(file)"
              :class="{ 'cursor-pointer hover:bg-muted/50': file.isFolder }"
              >{{ formatDate(file.lastModified) }}
            </TableCell>
            <TableCell class="text-center md:w-[110px]">
              <span v-if="!canPlay(file) && !showButtons"> - </span>
              <Button
                variant="ghost"
                size="icon"
                v-if="canPlay(file)"
  ...
  />
  <ConfirmModal
    :open="showFileCheckConfirmModal"
    :onOpenChange="(open: any) => (showFileCheckConfirmModal = open)"
    :onConfirm="performFileCheck"
    :title="t('media.file.confirmCheck.title')"
    :description="t('media.file.confirmCheck.desc')"
  />
</template>
...
const openConfirmModal = async (file: FileInfo) => {
  ...
  if (file.isFolder) {
    try {
      const res: any = await getEmptyDir({
        path: `${currentPath.value}/${file.filename}`.replace(/^\.\//, ''),
      });
      if (res.code !== 0) {
