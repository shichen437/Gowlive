<template>
        ...
        <TableHeader>
          <TableRow>
            <TableHead class="text-center">{{
              t('media.common.fields.filename')
            }}</TableHead>
            <TableHead class="text-center">{{
              t('media.check.fields.progress.title')
            }}</TableHead>
            <TableHead class="text-center">{{
              t('media.check.fields.status.title')
            }}</TableHead>
            <TableHead class="text-center">{{
              t('media.common.fields.duration')
            }}</TableHead>
            <TableHead class="text-center">{{
              t('media.check.fields.finishTime')
            }}</TableHead>
            <TableHead class="text-center">{{
              t('common.operation.title')
            }}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          ...
          <TableRow v-for="item in list" :key="item.id">
            ...
            <TableCell class="text-center"
              >{{ item.duration }}{{ t('common.fields.second') }}</TableCell
            >
  ...
  <ConfirmModal
    ...
    :onOpenChange="(open: any) => (showConfirmModal = open)"
    :onConfirm="performDelete"
    :title="t('common.operation.deleteConfirm')"
    :description="t('media.check.deleteConfirmDesc')"
  />
  <ConfirmModal
    :open="showConfirmDeleteAllModal"
    :onOpenChange="(open: any) => (showConfirmDeleteAllModal = open)"
    :onConfirm="handleDeleteAllTasks"
    :title="t('media.check.clear.title')"
...
function getFileStatusDisplay(status: number): {
  text: string;
  ...
  switch (status) {
    case 0:
      return {
        text: t('media.check.fields.status.pending'),
        variant: 'default',
      };
    case 1:
      return {
        text: t('media.check.fields.status.normal'),
        variant: 'outline',
      };
    case 2:
      return {
        text: t('media.check.fields.status.bad'),
        variant: 'destructive',
      };
    case 3:
      return {
        text: t('media.check.fields.status.notExist'),
        variant: 'default',
      };
    default:
      return {
        text: t('media.check.fields.status.unknown'),
        variant: 'outline',
      };
  }
}

function getProgressDisplay(progress: number): {
  text: string;
  variant: 'default' | 'secondary' | 'destructive' | 'outline';
} {
  switch (progress) {
    case 0:
      return {
        text: t('media.check.fields.progress.pending'),
        variant: 'default',
      };
    case 1:
      return {
        text: t('media.check.fields.progress.qcing'),
        variant: 'secondary',
      };
    case 2:
      return {
        text: t('media.check.fields.progress.qcerr'),
        variant: 'destructive',
      };
    case 3:
      return {
        text: t('media.check.fields.progress.acing'),
        variant: 'secondary',
      };
    case 4:
      return {
        text: t('media.check.fields.progress.acerr'),
        variant: 'destructive',
      };
    case 5:
      return {
        text: t('media.check.fields.progress.success'),
        variant: 'outline',
      };
    default:
      return {
