<template>
  <header
    class="flex h-18 items-center justify-between border-b bg-background px-4 md:px-6"
  >
        ...
        <HoverCardContent class="w-80">
          <div class="flex justify-between space-x-4">
            <div class="space-y-1">
              ...
              <div
                v-for="(category, key) in features.info"
                :key="key"
                class="text-sm"
              >
    ...
    onMessage: (msg) => {
      ...
      if (msg.data.notify) {
        if (msg.data.notify.level === 'info') {
          toast.info(msg.data.notify.title, {
            position: 'top-center',
            description: msg.data.notify.content,
          });
        } else {
