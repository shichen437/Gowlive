    ...
    <DialogContent class="sm:max-w-[425px]">
      <DialogHeader>
        ...
        <DialogDescription>
          {{
            isEditMode
              ? t('system.proxy.edit.desc')
              : t('system.proxy.add.desc')
          }}
        </DialogDescription>
      </DialogHeader>
      <div class="grid gap-4 py-4">
        <div class="grid grid-cols-4 items-center gap-4">
          <Label for="platform" class="text-right">{{
            t('common.fields.platform')
          }}</Label>
          <Select v-model="form.platform">
            <SelectTrigger class="col-span-3 w-full">
              <SelectValue
                :placeholder="t('stream.cookie.placeholder.platform')"
              />
            </SelectTrigger>
            <SelectContent>
              <SelectItem
        ...
        <div class="grid grid-cols-4 items-center gap-4">
          <Label for="proxy" class="text-right">{{
            t('system.proxy.fields.proxy')
          }}</Label>
          <Textarea
            id="proxy"
            v-model="proxyText"
        ...
        <div class="grid grid-cols-4 items-center gap-4">
          <Label for="remark" class="text-right">{{
